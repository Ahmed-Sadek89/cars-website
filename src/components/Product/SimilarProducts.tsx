...
const SimilarProducts = () => {
    return (
        <div className='custom-container1 flex flex-col gap-6 text-custom-black py-10'>
            ...
            <div className='grid grid-cols-5 items-center gap-x-16 gap-y-10'>
                {
                    additionalItems.map((item, index) => (
                        ...
                        >
                            <Image src={item.image} alt={item.title} width={200} height={200} className='object-contain w-[200px] h-[200px]' />
                            <h3>{item.title}</h3>
                        </Link>
                    ))
                }
